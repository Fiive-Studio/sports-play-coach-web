'use strict';
app.controller('reservationmultipleController', ['$scope', '$route', '$location', 'reservationmultipleService', 'localStorageService', 'authService', 'notificationService', function ($scope, $route, $location, reservationmultipleService, localStorageService, authService, notificationService) {
    ...
    $scope.onTimeSet = function (newDate, oldDate) {
        ...
        $scope.dateDisplay = moment($scope.dateNow).format("dddd") + ", " + moment($scope.dateNow).format("LL");
        fecha_reserva = fecha;
        reservationmultipleService.getBusy(authData.id_place, fecha).then(function (result) {
    ...
    if (!authData) {
        ...
        // Validamos si el establecimiento tiene canchas multiples
        reservationmultipleService.getIsMultiple(authData.id_place).then(function (result) {

            isMultiple = result.data;

            if (isMultiple == "True") {
                $("#reservaMultiple").show();
                $("#reservaMultipleFalse").html("");

                // Cargamos una sola vez la informacion de los horarios para generar las filas de la grilla
                reservationmultipleService.getPlace(authData.id_place).then(function (result) {
                    if (result.status != 200) {
                ...
                $("#tableScheduler").hide();
                $scope.$emit('LOAD');
                reservationmultipleService.getPitch(authData.id_place).then(function (results) {
                ...
                $scope.Reservar = function (pitch, hour) {
                    ...
                    reservationmultipleService.getPrice(pitch, hour, fecha_reserva).then(function (result) {
                        if (result.status != "200") {
                ...
                $scope.VerDetalle = function (pitch, id_reserva, hour, source) {
                    ...
                    if (source == '2') {
                        ...
                        $scope.messageChangeStatus = "Cancha reservada en la opcion normal.";
                        ...
                        reservationmultipleService.listStatus().then(function (result) {
                    ...
                    reservationmultipleService.getDetail(id_reserva).then(function (result) {
                        if (result.status != 200) {
                    ...
                    if ($scope.selectedReservationStatus != null) {
                        ...
                        if (valueFormViewReservation == null) {
                            ...
                            if ((valueFormViewReservation == 0 || valueFormViewReservation == "" || typeof valueFormViewReservation === 'undefined') &&
                               (observationFormViewReservation == "" || typeof observationFormViewReservation === 'undefined')) {
                                ...
                                reservationmultipleService.saveStatus($scope.selectedReservationStatus, $scope.reserva, observationFormViewReservation, valueFormViewReservation).then(function (result) {
                        ...
                        if (valueFormViewReservation == null) {
                            ...
                            if (valueFormViewReservation == 0 &&
                                ...
                                reservationmultipleService.saveStatus(idEstadoReservacion, $scope.reserva, observationFormViewReservation, valueFormViewReservation).then(function (result) {
                                    $('#VerDetalle').modal('toggle');
                                    reloadGrid();
                                }, function (error) {
                                    if (error.status == 401) { authService.logOut(); $location.path('/login'); }
                ...
                $scope.localSearch = function (str) {
                    ...
                    reservationmultipleService.getUserByNameAndIdPlace(str, authData.id_place).then(function (result) {
                        if (result.status != 200) {
                        ...
                        if ((price == 0 || price == "" || typeof price === 'undefined') && (observaciones == "" || typeof observaciones === 'undefined')) {
                            ...
                            if ($scope.idUser == "" || typeof $scope.idUser === 'undefined') {
                                ...
                                if (isNaN(phone) || !expresion.test(phone)) {
                                    ...
                                    reservationmultipleService.saveNewUser(name, phone, email, id_place).then(function (result) {
                                        if (result.status == 201) {
                                            var id_users_offline = result.data.id_users_offline;
                                            reservationmultipleService.saveReservationUserOffline(fecha_reserva, id_users_offline, id_pitch, hour, id_place, price, userType, observaciones).then(function (result) {
                                ...
                                reservationmultipleService.saveReservationUserOffline(fecha_reserva, id_users, id_pitch, hour, id_place, price, userType, observaciones).then(function (result) {
                ...
                $("#reservaMultiple").hide();
                $("#reservaMultipleFalse").html("<br><div class='alert alert-warning'>No se encontraron canchas multiples configuradas...</div>");
            }
    ...
    function reloadGrid() {
        $scope.$emit('LOAD')
        reservationmultipleService.getBusy(authData.id_place, $scope.dateNow).then(function (result) {
            if (result.status != 200) {
                $scope.message = "Upps! Lo siento, creo que tenemos un error al consultar las canchas ocupadas.";
                console.log(result);
            }
    ...
    function ShowNotificationUsersApp(id_place, fecha) {
        ...
        PNotify.removeAll();
        reservationmultipleService.getUserAppReservationMultiple(id_place, fecha).then(function (result) {
            if (result.status != 200) {
                $scope.message = "Upps! Lo siento, creo que tenemos un error al consultar las canchas ocupadas.";
                console.log(result);
            }
                ...
                if (result.data.length > 0) {
                    ...
                    $.each(result.data, function (index, item) {
                        ...
                        // Consultar datos detallados de la reserva
                        reservationmultipleService.getDetail(item.id_reservation).then(function (result) {
                            if (result.status != 200) {
                                $scope.message = "Upps! Lo siento, creo que tenemos un error al consultar detalle de la reserva.";
                                console.log(result);
                            }
